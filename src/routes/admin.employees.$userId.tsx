...
// ──────────────────────────────────────────────────────────────────────────────
// Individueller Arbeitsvertrag pro Mitarbeiter (Editor oder PDF-Upload)
// ──────────────────────────────────────────────────────────────────────────────

function ContractOverrideCard({ userId, fullName }: { userId: string; fullName: string }) {
  const { toast } = useToast();
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getOv({ data: { user_id: userId } });
      setExisting(res.override);
      if (res.override?.html_body) {
        setHtml(res.override.html_body);
        setMode("editor");
      } else if (res.override?.pdf_url) {
        setMode("pdf");
        const { data } = await supabase.storage.from("documents").createSignedUrl(res.override.pdf_url, 600);
  ...
  const handleUploadPdf = async (file: File) => {
    ...
    try {
      ...
      toast({ title: "PDF hochgeladen", description: "Mitarbeiter sieht ihn beim nächsten Login zur Unterschrift." });
      await reload();
    } catch (e: any) {
    ...
    <Card className="mt-4 border-primary/30">
      <CardContent className="pt-5 pb-5 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Individueller Arbeitsvertrag</p>
            <p className="text-[11px] text-muted-foreground mt-1">
              Überschreibt die Standard-Tenant-Vorlage nur für <strong>{fullName}</strong>. Mitarbeiter muss neu unterschreiben.
            </p>
          </div>
          {existing && (
            <Button size="sm" variant="ghost" className="text-xs text-destructive" onClick={handleDelete}>
              <Trash2 className="h-3.5 w-3.5 mr-1" /> Override entfernen
            </Button>
          )}
        </div>
        ...
        {existing && (
          <div className="flex items-center gap-2 text-[11px] text-accent bg-accent/10 px-3 py-2 rounded-lg">
            ...
            <span>
              Override aktiv ({existing.html_body ? "Text-Vertrag" : "PDF"}) – zuletzt aktualisiert{" "}
              {new Date(existing.updated_at).toLocaleString("de-DE")}
            </span>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              />
              {uploading && <p className="text-[11px] text-muted-foreground mt-1"><Loader2 className="h-3 w-3 inline animate-spin mr-1" /> Wird hochgeladen…</p>}
            </div>
            <p className="text-[11px] text-muted-foreground">
              Der Mitarbeiter sieht das PDF beim nächsten Login, kann es herunterladen und digital unterschreiben.
            </p>
          </div>
        )}
      </CardContent>
